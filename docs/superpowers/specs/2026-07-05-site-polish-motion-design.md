...
**Bileşen tedarik stratejisi (revize, 2026-07-06):** Lisans kapısı tetiklendi — React Bits lisansı **MIT + Commons Clause** çıktı (bileşenlerin bileşen olarak yeniden dağıtımı yasak) ve proje repo'su herkese açık. Kullanıcı kararıyla RB bileşenleri (BlurText, Magnet, CircularText) **temiz-oda yeniden yazım** ile bizim kodumuz olarak yazılır (`components/{blur-text,magnet,circular-text}.tsx`, reactbits.dev'e ilham atfı yorum satırıyla); yalnız düz-MIT olan Magic UI Marquee gerçek kaynakla vendorlanır (`components/ui/marquee.tsx`). API'ler: BlurText `{text, as?, delay?, animateBy?, className?}`, Magnet `{children, maxShift?, className?}`, CircularText `{text, spinDuration?, className?}`. Özel parçalar (köpük ayracı, köpük patlaması, akan çizgi, açık/kapalı nabzı) plandaki gibi bizim. WebGL kapsam dışı.
