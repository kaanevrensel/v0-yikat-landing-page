...
- `images.unoptimized: true` in next.config.mjs is deliberate (kept from the pre-pivot config). Hero sahne görselleri `<picture>` art-direction ile tek varyant indirir. Scrub videosu ORİJİNAL kalitede (1920×1080, 9.2MB — sahip kararı 2026-07-08: kalite > ağırlık; sıkıştırma denemesi geri alındı), `src` window `load` sonrası atanır. Mobil keyframe'ler Higgsfield outpaint+downscale ile 9:16 1170×2096 (ayakkabı kadraja tam sığar, retina-net); mobilde sahne metinleri CLEAR liquid-glass kartta (`hero-glass-card`, reduced-transparency kemeri globals.css'te); md+ ve hizmet sayfası girişinde kart YOK — görsel üstündeki yazıların okunabilirliğini harf konturu taşır (`.text-contour-dark/light`, globals.css: 8 yönlü em-tabanlı text-shadow, 2026-07-12 sahip isteği — kart çerçevesi denendi ve geri alındı, istenen harf sınırında konturdu).
